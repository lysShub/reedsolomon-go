...
func mulVect_simd(c byte, i []byte, o []byte) {
	n := len(i)
	switch c {
	case 0:
		clear(o[:n])
	...
	default:
		...
		if n >= 64 {
			...
			for n >= 64 {
				...
				lookupOrZero(loT, l0).Xor(lookupOrZero(hiT, h0)).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*0)))
				lookupOrZero(loT, l1).Xor(lookupOrZero(hiT, h1)).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*1)))
				lookupOrZero(loT, l2).Xor(lookupOrZero(hiT, h2)).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*2)))
				lookupOrZero(loT, l3).Xor(lookupOrZero(hiT, h3)).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*3)))
			...
			for n >= 16 {
				...
				lookupOrZero(loT, l0).Xor(lookupOrZero(hiT, h0)).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 0)))
...
func mulXorVect_simd(c byte, i []byte, o []byte) {
	n := len(i)
	switch c {
	case 0:
	case 1:
	...
	default:
		...
		if n >= 64 {
			...
			for n >= 64 {
				...
				lookupOrZero(loT, l0).Xor(lookupOrZero(hiT, h0)).Xor(e0).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*0)))
				lookupOrZero(loT, l1).Xor(lookupOrZero(hiT, h1)).Xor(e1).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*1)))
				lookupOrZero(loT, l2).Xor(lookupOrZero(hiT, h2)).Xor(e2).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*2)))
				lookupOrZero(loT, l3).Xor(lookupOrZero(hiT, h3)).Xor(e3).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 16*3)))
			...
			for n >= 16 {
				...
				lookupOrZero(loT, l0).Xor(lookupOrZero(hiT, h0)).Xor(e0).StoreArray((*[16]uint8)(unsafe.Add(oPtr, 0)))
...
func xorVect_simd(i []byte, o []byte) {
	n := len(i)
	iPtr := unsafe.Pointer(unsafe.SliceData(i))
	oPtr := unsafe.Pointer(unsafe.SliceData(o))
...
func lastIndex_simd(s []byte, v byte) int {
	n := len(s)
	if n == 0 {
		return -1
	}
	...
	for n >= 64 {
		...
		if maskNonZero(m0.Or(m1).Or(m2).Or(m3)) {
